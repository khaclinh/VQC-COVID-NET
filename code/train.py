...
def train(args, model, trainloader, optimizer, epoch):
    ...
    for input_tensors in tqdm(trainloader):
        ...
        for i in range(len(input_data)):
            if site[i] == 'ucsd':
                if len(ucsd_input) == 0:
                    ...
                    ucsd_label = torch.from_numpy(np.array([target[i].cpu().detach().numpy()]))
                    print("label = ", ucsd_label.numpy())
                else:
                    ucsd_input = torch.cat((ucsd_input, input_data[i].unsqueeze(0)))
                    ucsd_label = torch.cat((ucsd_label, torch.from_numpy(np.array([target[i].cpu().detach().numpy()]))))
            else:
                if len(new_input) == 0:
                    new_input = input_data[i].unsqueeze(0)
                    new_label = torch.from_numpy(np.array([target[i].cpu().detach().numpy()]))
                else:
                    new_input = torch.cat((new_input, input_data[i].unsqueeze(0)))
                    new_label = torch.cat((new_label, torch.from_numpy(np.array([target[i].cpu().detach().numpy()]))))

        if len(ucsd_input) > 1:
            ucsd_output, ucsd_features, ucsd_embedding_loss = model(ucsd_input, 'ucsd')
        if len(new_input) > 1:
            new_output, new_features, new_embedding_loss = model(new_input, 'ucsd')
        ...
        if len(ucsd_input) > 1 and len(new_input) > 1:
            ...
            embedding_loss = ucsd_embedding_loss + new_embedding_loss

        elif len(ucsd_input) > 1 and len(new_input) < 2:
            output = ucsd_output
            labels = ucsd_label.cuda()
            features = ucsd_features
            embedding_loss = ucsd_embedding_loss
        else:
            output = new_output
            labels = new_label.cuda()
            features = new_features
            embedding_loss = new_embedding_loss
        ...
        if len(features) == 32 and args.cont:
            ...
            loss = criterion(output, labels) + cont_loss + embedding_loss
        else:
...
def validation(args, model, testloader, epoch, mode='test'):
    ...
    with torch.no_grad():
        for input_tensors in tqdm(testloader):
            ...
            if len(ucsd_input) > 1:
                ucsd_output, ucsd_features, _ = model(ucsd_input, 'ucsd')
            ...
            if len(new_input) > 1:
                new_output, new_features, _ = model(new_input, 'ucsd')
